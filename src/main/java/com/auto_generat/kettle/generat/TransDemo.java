/*
 * @Author: wangran
 * @Date: 2020-06-09 13:00:47
 * @LastEditors: wangran
 * @LastEditTime: 2020-06-10 14:22:21
 */
...
public class TransDemo {
     ...
     * 两个库中的表名
     */
    public static String bjdt_tablename = "test_table";
    public static String kettle_tablename = "new_table";

    /**
     * 数据库连接信息,适用于DatabaseMeta其中 一个构造器DatabaseMeta(String xml)
     */
    public static final String[] databasesXML = {
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<connection>" + "<name>bjdt</name>"
                    + "<server>127.0.0.1</server>" + "<type>MYSQL</type>" + "<access>Native</access>"
                    + "<database>range</database>" + "<port>3306</port>" + "<username>root</username>"
                    + "<password>root</password>" + "</connection>",
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<connection>" + "<name>kettle</name>"
                    + "<server>127.0.0.1</server>" + "<type>MYSQL</type>" + "<access>Native</access>"
                    + "<database>range</database>" + "<port>3306</port>" + "<username>root</username>"
                    + "<password>root</password>" + "</connection>" };
    ...
    public static void main(String[] args) {
        try {
            ...
            String transName = "C:\\Users\\Administrator\\Desktop\\deskTopFolder\\copy-one-table.ktr";
